    ...
    public void init() {
        ...
        for (int i = 0; i < emplacements.size(); i++) {
            ...
            if (i == this.gamePlayer.getPlayerNumber()) {
                this.addItem(new ItemBuilder(Material.STAINED_CLAY, i+1, (short)1)
                        .withName(this.getIslandName(i))
                        .withLore(getIslandLore(IslandStatus.SELF_OCCUPIED))
                        .build());
            } else if (emplacement) {
                this.addItem(new ItemBuilder(Material.STAINED_CLAY, i+1, (short)14)
                        .withName(this.getIslandName(i))
                        .withLore(this.getIslandLore(IslandStatus.OCCUPIED))
                        .build());
            } else {
                int finalI = i;
                this.addItem(new ItemBuilder(Material.STAINED_CLAY, i+1, (short)5)
                        .withName(this.getIslandName(i))
